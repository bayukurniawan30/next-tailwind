            ...
            <div className="relative pt-6 px-4 sm:px-6 lg:px-8">
                <nav className="relative flex items-center justify-between sm:h-10 lg:justify-start" aria-label="Global">
                    ...
                    <div className="hidden md:block md:ml-10 md:pr-4 md:space-x-8">
                        {navMenu.map(({ route, title }) => (
                            <Link href={route} key={title}>
                                <a className="font-medium text-gray-500 hover:text-gray-900">{title}</a>
                            </Link>
                        ))}
                    </div>
                </nav>
                ...
                <div className="rounded-lg shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
                    ...
                    <div role="menu" aria-orientation="vertical" aria-labelledby="main-menu">
                        <div className="px-2 pt-2 pb-3 space-y-1" role="none">
                            {navMenu.map(({ route, title }) => (
                                <Link href={route} key={title}>
                                    <a className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50">{title}</a>
                                </Link>
                            ))}
                        </div>
                    </div>
